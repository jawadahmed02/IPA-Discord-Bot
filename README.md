...
- thread creation and member/thread helper flows
...
The bot is designed to run as the `IPA_Discbot.bot` package. Its startup path initializes the database, loads environment variables, constructs the Discord bot, registers handlers, and connects to both MCP servers before serving requests.
